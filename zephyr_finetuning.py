...
def main():
    ...
    def evaluate_question_answering(dataset, model_name):
        ...
        processed_preds = [pred[0]["generated_text"][pred[0]["generated_text"].find("<|assistant|> ") + len("<|assistant|> "):] for pred in preds]
        print(processed_preds)
        temp = []
        i = 0
        for pred in processed_preds:
            temp.append({"generated_text" : pred,
            "id" : dataset["test"]["id"][i]})
            i = i+1
        
        processed_preds = temp
        ...
        print(processed_labels[5])
        print(processed_preds[5])
